...
// ─── Service URLs ────────────────────────────────────────────
const FITNESS_URL =
  process.env.REACT_APP_FITNESS_URL || "http://localhost:3001";
const MERCHANT_URL =
  process.env.REACT_APP_MERCHANT_URL || "http://localhost:3002";
const GEO_URL = process.env.REACT_APP_GEO_URL || "http://localhost:3003";
const REWARDS_URL =
  process.env.REACT_APP_REWARDS_URL || "http://localhost:3004";
  ...
  return (
    <div className="ring-wrap">
      ...
      <div className="ring-legend">
        <span className="leg-dot" style={{ background: "#ff6b6b" }} />
        <span>{ingested} eaten</span>
        <span className="leg-dot" style={{ background: "#00d4aa" }} />
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <input
            placeholder="e.g. Chicken rice, Milo"
            value={desc}
...
function FitnessModal({ userId, onClose, onSync }) {
  ...
  const pts = form.distance_miles
    ? Math.round(parseFloat(form.distance_miles) * 10)
    : 0;
  const cal = form.distance_miles
    ? Math.round(parseFloat(form.distance_miles) * 80)
    : 0;
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setForm({ ...form, distance_miles: e.target.value })
            }
          />
        </div>
        {pts > 0 && (
          <div className="preview-box">
            <div>
              ⭐ <strong>+{pts}</strong> Quest Points
            </div>
            <div>
              🔥 <strong>~{cal}</strong> cal burned
            </div>
          </div>
        )}
        <div className="modal-actions">
      ...
      <div className="drawer">
        ...
        <div className="drawer-section">
          ...
          <p className="drawer-hint">
            Port 3001 · Activities synced automatically
          </p>
        </div>
    ...
    <div className="app">
      ...
      <main className="main" style={{ textAlign: "center", paddingTop: 60 }}>
        ...
        <p style={{ color: "var(--text-muted)", marginTop: 8 }}>
          Stall Owner tools coming soon.
          <br />
          Merchant dashboard in Phase 2.
        </p>
...
function StudentApp({ user, onLogout }) {
  ...
  const [calorieGoal, setCalorieGoal] = useState(
    () => parseInt(localStorage.getItem(`cg_${user.id}`)) || 2000,
  );
  const [ingested, setIngested] = useState(
    () => parseInt(localStorage.getItem(`ing_${user.id}_${today()}`)) || 0,
  );
  ...
  const [notifs, setNotifs] = useState(
    () => localStorage.getItem("notifs") !== "off",
  );
  ...
  useEffect(() => {
    api(`${FITNESS_URL}/api/fitness/user/${user.id}`).then((d) => {
      ...
      if (d.stats?.total_calories)
        setBurned(parseInt(d.stats.total_calories) || 0);
    });
    api(`${FITNESS_URL}/api/fitness/leaderboard`).then((d) => {
      if (d.leaderboard) setLeaderboard(d.leaderboard);
    });
    api(`${MERCHANT_URL}/api/merchant/canteens`).then((d) => {
      if (d.canteens) setCanteens(d.canteens);
    });
    api(`${REWARDS_URL}/api/rewards/merchandise`).then((d) => {
      if (d.merchandise) setMerchandise(d.merchandise);
    });
  }, [user.id]);
  ...
  useEffect(() => {
    localStorage.setItem(`cg_${user.id}`, calorieGoal);
  }, [calorieGoal, user.id]);
  ...
  const handleMealLog = (cal, desc) => {
    ...
    showToast(
      `${meal.emoji} Logged ${cal} cal${burning ? " — burning more than eating! +bonus pts 🔥" : ""}!`,
    );
  };
  ...
  const handleFitnessSync = (data) => {
    ...
    setUserData((prev) =>
      prev
        ? {
            ...prev,
            user: { ...prev.user, quest_points: data.new_total_points },
          }
        : prev,
    );
  ...
  return (
    <div className="app">
      ...
      {showFitness && (
        <FitnessModal
          userId={user.id}
          onClose={() => setShowFitness(false)}
          onSync={handleFitnessSync}
        />
      )}
      ...
      {showSettings && (
        <SettingsDrawer
          ...
          onNotifToggle={() => {
            setNotifs((n) => !n);
            localStorage.setItem("notifs", notifs ? "off" : "on");
          }}
        ...
        <main className="main tab-content">
          ...
          {bonusPts > 0 && (
            <div className="bonus-banner">
              🔥 You've burned more than you ate today! Bonus{" "}
              <strong>+{bonusPts} pts</strong> unlocked!
            </div>
          )}
          ...
          <div className="leaderboard">
            {leaderboard.map((u) => (
              <div
                key={u.id}
                className={`leaderboard-row ${u.id === user.id ? "mine" : ""} ${u.rank === 1 ? "gold" : u.rank === 2 ? "silver" : u.rank === 3 ? "bronze" : ""}`}
              >
          ...
          {myRank && (
            <p className="your-rank">
              You are ranked <strong>#{myRank.rank}</strong> 🎯
            </p>
          )}
        </main>
      )}
                  ...
                  <div className="act-icon">
                    {a.activity_type === "running"
                      ...
                      : a.activity_type === "cycling"
                        ? "🚴"
                        : a.activity_type === "swimming"
                          ? "🏊"
                          : "🚶"}
                  </div>
                  <div className="act-info">
                    <p className="act-type">{a.activity_type}</p>
                    <p className="act-details">
                      {a.distance_miles} mi · {a.calories_burned} cal
                    </p>
      ...
      {activeTab === "location" && (
        <main className="main tab-content">
          ...
          <p className="subtitle">
            Walk within 50m of a canteen to unlock its menu
          </p>
          <button
            className="btn btn-primary"
            style={{ marginBottom: 20 }}
            onClick={async () => {
              ...
              navigator.geolocation.getCurrentPosition(
                async (pos) => {
                  ...
                  showToast(
                    d.message || "Location checked",
                    d.within_range ? "success" : "info",
                  );
                },
          ...
          <div className="canteen-grid">
            {canteens.map((c) => (
              <div key={c.id} className="canteen-card">
                ...
                <span
                  className={`status-badge ${c.is_open ? "open" : "closed"}`}
                >
          ...
          <div className="merch-grid">
            {merchandise.map((item) => (
              <div key={item.id} className="merch-card">
                ...
                >
                  {questPoints >= item.cost_in_points
                    ? "Claim 🎉"
                    : "Need more pts"}
                </button>
              </div>
      ...
      <footer className="footer">
        🎓 CampusQuest Go · Republic Polytechnic 2026
      </footer>
    </div>
  );
}
...
export default function App() {
  const [currentUser, setCurrentUser] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cq_user"));
    } catch {
      return null;
    }
  });
  ...
  if (currentUser.role === "stall_owner")
    return <StallOwnerApp user={currentUser} onLogout={handleLogout} />;
