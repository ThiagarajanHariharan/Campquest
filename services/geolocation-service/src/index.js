...
app.post('/api/geo/check-location', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to check location' /* sanitized to prevent information leakage */ });
  }
});
...
app.get('/api/geo/user/:userId/current-location', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch user location' /* sanitized to prevent information leakage */ });
  }
});
