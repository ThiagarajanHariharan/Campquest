...
app.post('/api/geo/check-location', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to check location', details: 'Internal Server Error' /* Security: Prevent internal info leakage */ });
  }
});
...
app.get('/api/geo/user/:userId/current-location', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch user location', details: 'Internal Server Error' /* Security: Prevent internal info leakage */ });
  }
});
