...
app.use((req, res, next) => {
  console.log(
    `[Fitness-Sync] ${req.method} ${req.path} - ${new Date().toISOString()}`,
  );
  next();
});
...
app.post("/api/fitness/sync", async (req, res) => {
  ...
  if (distance_miles <= 0) {
    return res
      .status(400)
      .json({ error: "distance_miles must be greater than 0" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to sync fitness activity", details: err.message });
  } finally {
    client.release();
  }
});
...
app.get("/api/fitness/user/:userId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch user data", details: err.message });
  }
});
...
app.get("/api/fitness/activities", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch activities", details: err.message });
  }
});
