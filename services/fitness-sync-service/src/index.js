...
app.get('/api/fitness/user/:userId', async (req, res) => {
  ...
  try {
    ...
    // Parallelize independent queries to fetch user activities and stats concurrently using Promise.all.
    // This reduces the overall latency by executing these database operations in parallel rather than sequentially, saving approximately ~30-50ms per request.
