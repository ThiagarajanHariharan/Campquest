...
app.post('/api/fitness/sync', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to sync fitness activity' /* sanitized to prevent information leakage */ });
  } finally {
    client.release();
  }
});
...
app.get('/api/fitness/user/:userId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch user data' /* sanitized to prevent information leakage */ });
  }
});
...
app.get('/api/fitness/leaderboard', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch leaderboard' /* sanitized to prevent information leakage */ });
  }
});
...
app.post('/api/fitness/user', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to create user' /* sanitized to prevent information leakage */ });
  }
});
...
app.get('/api/fitness/activities', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch activities' /* sanitized to prevent information leakage */ });
  }
});
