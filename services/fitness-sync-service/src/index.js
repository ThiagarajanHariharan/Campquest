...
app.get('/api/fitness/user/:userId', async (req, res) => {
  ...
  try {
    ...
    // ⚡ Bolt: Run independent database queries concurrently using Promise.all
    // Expected impact: Reduces endpoint latency by executing queries in parallel rather than sequentially, saving one database round-trip time.
